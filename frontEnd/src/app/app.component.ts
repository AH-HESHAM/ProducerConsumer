...
export class AppComponent implements OnInit {
  ...
  simulate() {
    ...
    this.http.get(`http://localhost:9090/simulate/${productsNumber}`)
      .subscribe();
    console.log(productsNumber)
  }
  replay() {
    this.restart = true
    this.http.get("http://localhost:9090/replay").subscribe();
  }
  queueState() {
    this.counterQueue++
    let group = new Group({
  ...
  connectionState(logInForm: NgForm) {
    ...
    // }
    if (from[0] == 'M') {
      this.http.get("http://localhost:9090/connectMachineToQueue", {
        params: {
          machineId: f,
          queueId: t
        }
    ...
    if (from[0] == 'Q') {
      this.http.get("http://localhost:9090/connectQueueToMachine", {
        params: {
          queueId: f,
          machineId: t
        }
  ...
  clear() {
    this.http.get("http://localhost:9090/clear").subscribe();
    this.layer.destroy()
    this.layer = new Layer()
    this.stage.add(this.layer)
