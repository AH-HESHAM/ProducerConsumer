...
public class SingleTonDiagram extends Clone {
    ...
    private List<Integer> storedIDQueues = new ArrayList<>();

    public SingleTonDiagram getDiagramClone() {
        return diagramClone;
    }

    public void addQueueID(int id) {
        storedIDQueues.add(id);
    }
    ...
    public void replay(int n) {
        ...
        for (Product p : getQueues().get(getNumberOfQueues() - 1).getProducts()) {
            getQueues().get(0).add(p);

        }
        getQueues().get(getNumberOfQueues() - 1).getProducts().clear();
        diagram = SingleTonDiagram.getInstance();
        diagram.simulate(n);

        // for (int i = 0; i < storedIDQueues.size(); i++){
        // queues.set(i, new QueueOfProducts(storedIDQueues.get(i)));
        // }
